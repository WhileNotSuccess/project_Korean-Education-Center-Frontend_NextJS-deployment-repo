...
export default function SelectTabComponent({ name, categoryTab }: SelectTabProps) {
  ...
  const [courseOptions, setCourseOptions] = useState<Array<{ course: string }>>([ // 하드코딩된 과정 옵션
    { course: "지원과정 1" },
    { course: "지원과정 2" },
    { course: "지원과정 3" },
  ]); // 지원과정 목록 추가시 삭제 예정
    ...
    <div className="w-full h-screen">
      ...
      <div className="w-full flex justify-center mt-8">
        {selectedTab !== "upload-documents" ? (
          <div className="w-3/5 p-4">{typeof content === "string" ? parser(content) : "내용을 불러올 수 없습니다."}</div>
        ) : (
                ...
                >
                  {courseOptions.length > 0 ? (
                    courseOptions.map((option, index) => (
                      <option key={index} value={option.course}>{option.course}</option>
                    ))
                  ) : (
